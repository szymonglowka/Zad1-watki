            ...
            while True:
                ...
                if line == "login":
                    ...
                    while not login_pass:
                        ...
                        #login = line nie wiem co to robi ; P
                        s = line.split()
                    ...
                    login_pass = False
                    while not login_pass:
                        out.write("Login password nickname?\r\n")
                        out.flush()
                    ...
                    if login_pass:
                        for user in self.logins:
                            if user[0] == login_user:
                                ...
                                if int(transfer_amount) <= int(user[4]):
                                    ...
                                    print(thread_name + "| Line sent: Nie mozesz przelac")
                                    out.write("Nie mozesz przelac")
                                    out.flush()
                                    break
                        ...
                        for user in self.logins:
                            if user[0] == login_user:
                                ...
                                if int(line) <= int(user[4]):
                                    ...
                                    print(thread_name + "| Line sent: Jestes biedny")
                                    out.write("Jestes biedny")
                                    out.flush()
                                    break
...
def change_param_in_file(file_name, login, new_balanace):
    for linia in fileinput.input(file_name, inplace=True):
        param = linia.strip().split()  # Dzielimy linię na parametry po spacji
        if param[0] == login:  # Sprawdzamy, czy linia ma wystarczająco wiele parametrów i czy login pasuje
            param[4] = str(new_balanace)  # Zmieniamy piąty parametr na nowe saldo
            print(" ".join(param))  # Wypisujemy zmienioną linię
        else:
            print(linia.strip())
